...
class ActivityDpr extends Model
{
    ...
    public static function getAllDprActivityByUser($user_id,$site_id){
		 ...
		 $result = DB::table('activity_dprs')
            ->select('activity_dprs.id','activities.name as activity_name','activity_dprs.status',
            'activity_dprs.created_at',
            'activity_dprs.quantity','activity_dprs.rfi_no','activity_dprs.description',
            'activity_dprs.stretch_one',
            'activity_dprs.stretch_two','users.name as username','sites.name as site_name',
            'uoms.name as uom_name',DB::raw('CONCAT(activity_dpr_images.image) as images'))
    ...
    public static function getTotalActivity($site_id){        
        $result = DB::table('site_activities')
            ...
            'uoms.name as uom_name','uoms.id as uom_id','activity_categories.name as activity_category_name',
