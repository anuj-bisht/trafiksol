      ...
      <div class="modal-header">
        ...
        {!! Form::open(array('route' => 'sites.store','method'=>'POST','id'=>'site_sla_model_form')) !!}
          ...
          <div class="modal-footer">
            ...
            <button type="submit" class="btn btn-primary" id="assignSubmit">Assign</button>
          </div>
        {!! Form::close() !!}
    </div>
  </div>
        ...
        <div class="modal-body">
          ...
          <div class="col-xs-4 col-sm-4 col-md-3">
              <div class="form-group">
                  ...
                  {!! Form::text('equipment_chainage', null, array('placeholder' => 'Chainage','id'=>'chainage_id','class' => 'form-control')) !!}
              </div>
          </div>
          <div class="col-xs-4 col-sm-4 col-md-3">
              <div class="form-group">
                  <strong>Location:</strong>
                  {!! Form::select('location', ['LHS'=>'LHS','RHS'=>'RHS','Median'=>'Median','Plaza'=>'Plaza'],null, array('placeholder' => 'Location','id'=>'location_id','required'=>'true','class' => 'form-control')) !!}
              </div>
          </div>
          <div class="col-xs-4 col-sm-4 col-md-3">
              <div class="form-group">
                  <strong>Equipment Sla:</strong>
                  {!! Form::select('equipment_sla', $sla_list,null, array('placeholder' => 'SLA','id'=>'equi_id_sla','required'=>'true','class' => 'form-control')) !!}
              </div>
          </div>
      ...
      success: function(res) {
          if((res.status) && (res.data.length > 0)){
            ...
            jQuery.each(res.data, function(index, item) {
                ...
                html += "<td>"+item.equipment_location+"</td>";
                html += "<td>"+item.brand_name+"</td>";
                html += "<td>"+item.model_name+"</td>";
                html += "<td>"+item.sla_name+"</td>";
                html += "<td><span onclick='editSiteAssignment("+item.site_equipment_id+","+item.equipment_id+")' style='color:blue;cursor:pointer;' class='glyphicon glyphicon-edit'></span> || <span onclick='deleteSiteAssignment("+item.site_equipment_id+",\"equipment\")' style='color:red;cursor:pointer;' class='glyphicon glyphicon-trash'></span></td>";
                html += '</tr>';

            });
            $('#equipment_modal_tr').html(html);
            $('#site_title').html(res.data[0].site_name);
          }else{
            $('#equipment_modal_tr').html('<td colspan="6" style="text-align:center;"><i>No equipment assigned</i></td>');
          }
...
function editSiteAssignment(id,equip_id)
{
    $('#assign_btn').attr("disabled",true);
    var url = "{{url('/')}}/sites/getSiteAssignment/"+id;
  ...
  $.ajax({
      ...
      success: function(res) {
        //   console.log(res.status);
          if(res.status == 200){
            $('#id').val(id);
            $('#data_vendor_id').val(res.data[0].vendor_id);
            // $('#data_vendor_id').trigger("change");
            datavendorid(res.data[0].brand_id);
            mainbrand_id(res.data[0].model_id,res.data[0].brand_id);
            model_id(res.data[0].equipment_id,res.data[0].model_id);
            $('#chainage_id').val(res.data[0].chainage);
            $('#location_id').val(res.data[0].location);
            $('#equi_id_sla').val(res.data[0].sla_id);
...
function datavendorid(selected_id){





var data_vendor_id = document.getElementById('data_vendor_id').value;
// alert(selected);
...
function mainbrand_id(sel_id,main_brand_id)
{


// var main_brand_id = document.getElementById('main_brand_id').value;
    ...
    success: function(result){
        ...
        if(result.success){
            jQuery.each(result.data, function(index, item) {
                ...
                html += "<option value='"+item.id+"' "+selected+">"+item.model+'-'+item.make+'-'+item.build+"</option>";
            });
        }
...
function model_id(sel_id,model_id)
{
