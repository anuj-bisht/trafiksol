...
@section('content')
	...
	<div id="page-wrapper">
      ...
      <div class="row">
				<div class="col-lg-12">
					<h1 class="page-header">{{__('SLA Category')}}</h1>
				</div>
				<!-- /.col-lg-12 -->
			</div>  
      ...
      <div class="row">
        <div class="col-lg-12 margin-tb">
            ...
            <div class="pull-right">
                <a class="btn btn-success" href="{{ route('equipment_slas.create') }}"> Create New SLA Category</a>
            </div>
        </div>
      </div>
	...
	<!-- /#page-wrapper -->
  {!! Form::open(['method' => 'DELETE','route' => ['equipment_slas.destroy', 1],'id'=>'deleteRow','style'=>'display:inline']) !!}
  ...
  <script>
    ...
    jQuery(document).ready(function() {
          ...
          table = jQuery('#tableData').DataTable({
            ...
            'ajax': {
              'url': '{{ url("/") }}/equipment_slas/ajaxData',
              'headers': {
            ...
            'columns': [
              ...
              {
                  'data': 'hours_allocated',
                  'orderable': false,
                  'className': 'col-md-2',
                  'render': function(data,type,row){
                    
                    return row.hours_allocated;
                  }
              },
              {
                  'data': 'sla_type',
              ...
              },            
              {
                ...
                'className': 'col-md-2',
                'render': function(data, type, row) {
                  var buttonHtml = '<a class="btn btn-primary" href="'+url+'/equipment_slas/'+row.id+'/edit">Edit</a>&nbsp;&nbsp;<a class="btn btn-danger" href="javascript:void(0);" onclick="deleteData('+row.id+')">Delete</a>';
                  return buttonHtml;
                }
              }
            ]
...
function deleteData(id){
  ...
  $.confirm({
      ...
      buttons: {
          confirm: function () {
            $('#deleteRow').attr('action', url+"/equipment_slas/"+id).submit();  
          },
          cancel: function () {
              return true;
          }
