...
@section('content')
	...
	<div id="page-wrapper">
		<div class="container-fluid">
            ...
            {!! Form::model($category_vendor, ['method' => 'PATCH','route' => ['category_vendors.update', $category_vendor->id]]) !!}            
            <div class="row">
                        ...
                        <select name="parent_id" class="form-control">
                            ...
                            @if(count($category) > 0 )
                               @foreach($category as $v)
                                    <option value="{{$v->id}}" @if($v->parent_id==$category_vendor->id) selected @endif>{{$v->name}}</option>
                               @endforeach     
                            @endif
                        <select>                        
                    </div>
