...
@section('content')
	...
	<div id="page-wrapper">
      ...
      <div class="row">
				<div class="col-lg-12">
					<h1 class="page-header">{{__('Vehicle DPR List')}}</h1>
				</div>
				<!-- /.col-lg-12 -->
			</div>  
  ...
  <script>
    ...
    jQuery(document).ready(function() {
          ...
          table = jQuery('#tableData').DataTable({
            ...
            {extend:'csvHtml5',
              exportOptions: {
                columns: [1,2,3,4,5,6,7,8,9]//"thead th:not(.noExport)"
              },
            ...
            'columns': [                         
              ...
              },
              {
                  ...
                  'render': function(data,type,row){
                    ...
                    if(images!=null){
                      return '<a href="'+row.image+'" data-fancybox="gallery'+row.id+'" style="text-align:center;font-size:18px"><i class="fa fa-camera" aria-hidden="true"></i></a>';
                    }else{
                        return '';
                    }
                  }
              },            
              {
                ...
                'className': 'col-md-1',
                'render': function(data, type, row) {
                  var buttonHtml = '<a class="btn btn-primary" href="'+url+'/vehicles/dprshow/'+row.id+'">show</a>';
                  return buttonHtml;
                }
              }
            ]
...
function isCheckboxChecked(val){
    ...
    $.confirm({
        ...
        buttons: {
            confirm: function () {
              var url = "{{url('/')}}/vehicles/approveRejectVehicle"; 
              $.ajax({
                  type: "POST",
                  url: url,      
                  data: {
