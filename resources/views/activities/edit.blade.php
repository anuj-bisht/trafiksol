...
@section('content')
	...
	<div id="page-wrapper">
		<div class="container-fluid">
            ...
            <div class="row">
                    <div class="col-lg-10">
                        <h1 class="page-header">{{__('Edit Activity')}}</h1>                        
                    </div>		                    
            </div>
            ...
            <div class="col-xs-12 col-sm-12 col-md-12">
                        ...
                        <select name="main_category" id="main_activity_category" class="form-control">
                            ...
                            @if(count($category) > 0 )
                               @foreach($category as $v)
                                    <option value="{{$v->id}}" @if($v->id==$selected_category) selected @endif>{{$v->name}}</option>
                               @endforeach     
                            @endif
                        <select>                        
                    </div>
                        ...
                        <select name="activity_category_id" id="activitySubCategoryeDD" class="form-control">
                            ...
                            @if(count($child_cat) > 0 )
                               @foreach($child_cat as $v)
                                    <option value="{{$v->id}}" @if($v->id==$selected_sub_category) selected @endif>{{$v->name}}</option>
                               @endforeach     
                            @endif                       
                        <select>                        
                    </div>
                ...
                <div class="col-xs-12 col-sm-12 col-md-12">
                    <div class="form-group">
                        <strong>Uom:</strong>
                        {!! Form::select('uom_id', $uomList,$activity->uom_id, array('class' => 'form-control','id'=>'selectUom','placeholder'=>'Select Uom')) !!}
                    </div>
                </div>
